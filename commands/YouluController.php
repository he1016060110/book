...
{
    /**
     * @var $_cache \yii\redis\Connection
     */
    private $_cache;
  ...
  UNIQUE KEY `book_id` (`book_id`)
    ...
    public function actionUpdateNum()
    {
        $curl = new Curl();
        $count = 0;
        while($book_id = $this->_cache->rpop("update-num-job")){
    ...
    public function actionPushUpdateNumJob()
    {
        $ids = YouLuSaleModel::find()->where("status=0")->column();
        foreach ($ids as $id) {
            $this->_cache->lpush("update-num-job", $id);
        }
    }
}